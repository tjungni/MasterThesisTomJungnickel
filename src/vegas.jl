# taken from MonteCarloIntegration.jl
using Random
using Distributions
...
function vegas(func, 
    ...
    iter = 1
	ymhelp = QuasiMonteCarlo.sample(ncalls, zeros(ndim), ones(ndim), QuasiMonteCarlo.UniformSample())	
	Js_final = zeros(eltype(ymhelp), size(ymhelp, 2))
	...
	VEGASResult(Itot, sd, chi_squared/(iter-1), x, delx, Js_final)
end
...
function sample_from_adaptive_grid(res, ncalls)
	...
	return (map(x -> tuple(x...), eachcol(xmat)), Js)
end
